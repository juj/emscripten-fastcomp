...
struct EmitFunctionGraphData : public FunctionPass {
  static char ID; // Pass identification, replacement for typeid
  std::string destinationFile;
  EmitFunctionGraphData(const std::string &destinationFile) : FunctionPass(ID), destinationFile(destinationFile)
  {
...
char EmitFunctionGraphData::ID = 0;
...
bool EmitFunctionGraphData::runOnFunction(Function &Func) {
  FunctionEntry fe;
...
extern FunctionPass *createEmitFunctionGraphDataPass(std::string outputJsonFile) {
  return new EmitFunctionGraphData(outputJsonFile);
}
